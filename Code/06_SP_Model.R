#### Script for Bayesian spatio-temporal models based on the full period
...
grid_5nn <- expand.grid(plt = c("no2", "pm10", "pm25"),
  med = c("BD", "ICS", "Rx")
)
  ...
  formula <- switch(
    ...
    "ICS" = formula <- as.formula(
      paste0(
        ...
        "CrimeRate + HouseOCrat + HouseNCrat + Chinese + Black + Humidity + Temperature +",
        toupper(i), "+ as.factor(Year) + as.factor(Month) +",
        "as.factor(Temp_Trend == 51) + as.factor(Temp_Trend == 52) +",
        "as.factor(Temp_Trend == 53) + as.factor(Temp_Trend == 54) +",
        "f(GP, model = 'besagproper2', graph = g, group = Temp_Trend,",
...
# Save results
save(list = c("data", names(model_results)), file = "./Results/Result_5nn.RData")

stopCluster(cl)

#### Modelling for Mat1 and Mat2 ###############################################
...
model_results <- foreach(
  ...
  file_name <- paste0("./Data/Matrices/", v, ".RData")
  load(file_name)
  ...
  save(list = c("data", names(results)), file = paste0("./Results/Result_", v, ".RData"))
  rm(list = ls())
  gc()
}
